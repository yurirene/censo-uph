...
class EstatisticaService
{
    ...
    public static function exportarExcel(array $request)
    {
        ...
        return Excel::download(
            new BaseDadosFormularioExport($campos, $somente_colunas, $request['ano_referencia']),
    ...
    public static function getDadosQualidadeEstatistica(): Collection
    {
        ...
        return Sinodal::where('status', true)
            ...
            ->map(function ($item) use ($ano) {
                ...
                $nome = str_replace(['Homem', 'Homens'], 'M', $nome);
                return [
                    'id' => $item->id,
                    'ano' => $ano,
                    'nome' => $nome,
    ...
    public static function getDadosRelatorioGeral($ano_referencia, $regiao_id = null): array
    {
        try {
            ...
            $totalizador['estrutura']['uphs_organizadas'] = Local::where('status', true)
                ->when(!is_null($regiao_id), function ($sql) use ($regiao_id) {
            ...
            $totalizador['abrangencia']['locais'] = [
                ...
                'total' => $totalizador['estrutura']['uphs_organizadas']
            ];

            $totalizador['qualidade'] = ($formularios_locais->count() * 100) / $totalizador['estrutura']['uphs_organizadas'];
